...
auto Window::WndProc(::UINT uMsg, ::WPARAM wParam, ::LPARAM lParam) -> ::LRESULT {
    switch (uMsg) {
        case WM_ACTIVATE:
            return on_activate(wParam, lParam);
        case WM_DESTROY:
        ...
        case WM_SYSKEYDOWN:
            return on_sys_key_down(wParam, lParam);
    }
...
auto Window::on_get_min_max_info(::WPARAM wParam, ::LPARAM lParam) -> int {
    ...
    DwmGetWindowAttribute(
        m_hwnd.get(), DWMWA_VISIBLE_FRAME_BORDER_THICKNESS, &border, sizeof(border));
...
auto Window::on_key_down(::WPARAM wParam, ::LPARAM lParam) -> int {
    ...
    if (m_keys.set.contains(key)) {
        switch (key) {
            ...
            case VK_F3: {
                if (m_layout.split) {
                    m_layout.vertical = !m_layout.vertical;
                }
...
auto Window::on_notify(::WPARAM wParam, ::LPARAM lParam) -> int {
    ...
    switch (notification->code) {
        ...
        case FAVICON_CHANGED: {
            ...
            std::function<::HRESULT()> secondCallback { [this]() {
...
auto Window::update_caption() -> void {
    if (!m_fullscreen) {
        if (!m_layout.swap) {
            set_icon(m_first.hicon.get(), true, false);
        }

        else {
            set_icon(m_second.hicon.get(), true, false);
        }
    ...
    if (!m_layout.swap) {
        set_title(m_first.title);
    }

    else {
        set_title(m_second.title);
    }
}
