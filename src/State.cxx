bool State::Toggle(bool b) { return b ? false : true; }

void State::MaximizeWindow(HWND window)
{
    if (!fullscreen)
    {
        if (!maximized)
...
void State::FullscreenWindow(HWND window)
{
...
void State::TopmostWindow(HWND window)
{
    FLASHWINFO fwi;
...
void State::UpdateBounds(HWND hwnd)
{
    if (main_controller != nullptr)
